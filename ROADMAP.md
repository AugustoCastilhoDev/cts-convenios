...
- [ ] **2FA (autenticação em dois fatores)** — próximo passo de segurança, **antes do primeiro cliente**. Vale para o **super administrador** e para o **administrador da prefeitura** (as duas contas com poder sobre o acesso de todos); opcional para gestor e fiscal. Pontos a decidir/fazer: app autenticador (TOTP), códigos de recuperação, o que fazer se o administrador perder o celular (o super administrador redefine o 2FA do administrador da prefeitura; para o super, comando `artisan`), obrigatoriedade no primeiro acesso, e teste de ponta a ponta do fluxo.
- [ ] **Validade da senha temporária** — hoje a senha gerada ao criar a conta ou em "Redefinir senha" não expira até o primeiro acesso. Proposta: guardar quando foi gerada e recusar o login depois de N dias (sugestão: 7) com a mensagem "senha temporária expirada, peça ao administrador para redefinir"; a lista de usuários passa a mostrar "temporária expirada" e o botão Redefinir gera outra. Decidir o prazo com o dono e cobrir com testes (PHP, Vitest e ponta a ponta).
