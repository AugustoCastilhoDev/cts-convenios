...
- **Endurecimento de produção (2026-09-25)**: (1) logs do Docker com rotação (3 x 10 MB por container) em `docker-compose.prod.yml`; (2) workflow `Dependências` (`composer audit` e `npm audit` só de produção, a cada mudança nos locks e toda segunda) + Dependabot; (3) **"Esqueci minha senha"**: `POST /api/esqueci-senha` (resposta sempre igual, contas desativadas não recebem, 3/min por IP e 5/h por e-mail) e `POST /api/redefinir-senha` (link de 60 min e uso único, derruba todas as sessões, mesma resposta para token errado/vencido/e-mail inexistente); o link usa `APP_URL`, nunca o `Host` da requisição; com `ALERTAS_REDIRECIONAR_PARA` o e-mail é redirecionado e marcado `[TESTE]`; (4) **senha temporária**: conta criada ou senha redefinida por um administrador liga `must_change_password`; enquanto ligada a API só libera `/me`, `/me/password` e sair (403 `senha_temporaria`) e a tela leva à troca; (5) **regra única de senha** (`Password::defaults()`): 10+ caracteres com letras e números e, em produção, recusa senhas vazadas (Have I Been Pwned, só um prefixo de hash é enviado). Telas: Esqueci a senha, Criar nova senha, Defina sua senha (layout compartilhado `TelaDeAcesso`). Testes: PHP, Vitest e ponta a ponta. Revisada com a skill `security-review`. **Ainda falta** (segurança): 2FA para o Administrador Interno, monitoramento de erros (Sentry), antivírus opcional nos uploads, ambiente de homologação, retenção da tabela de auditoria; convite por e-mail no lugar de o administrador digitar a senha inicial.
- **2FA e validade da senha temporária (2026-09-26)**: (1) **senha temporária** vence em 7 dias (`SENHA_TEMPORARIA_VALIDADE_DIAS`, `users.senha_temporaria_expira_em`): depois disso o login é recusado (só depois de a senha conferir), a lista mostra "senha temporária vencida", o aviso da senha informa até quando ela vale e "Redefinir senha" começa outro prazo; "Esqueci minha senha" continua funcionando; sem prazo gravado conta como vencida. (2) **Verificação em duas etapas por app autenticador (TOTP, `pragmarx/google2fa`)**: **obrigatória** para o super administrador e o administrador da prefeitura (`DOIS_FATORES_OBRIGATORIO`, `config/seguranca.php`; só o `.env` de desenvolvimento e o CI usam `false`), opcional para gestor e fiscal em "Segurança da conta". Ativação em 3 passos (senha, QR Code desenhado no navegador + chave, código) e 8 códigos de recuperação mostrados uma vez (só o hash com chave fica no banco); segredo criptografado com a `APP_KEY`, fora da auditoria e de qualquer resposta depois da ativação. Login em duas etapas: e-mail e senha corretos não emitem token, emitem um desafio de 5 min (5 tentativas; 10 erros por pessoa em 10 min bloqueiam; 10/min por IP) que só o código completa; cada código de 30 s entra uma vez (guarda o último período, tolerância de um para cada lado) e os de recuperação são de uso único. Até ativar, a API só libera perfil, troca de senha, 2FA e sair (403 `2fa_obrigatorio`; a troca da senha temporária vem antes). Ativar, gerar novos códigos, desativar e redefinir pedem a senha de novo (desativar também um código; perfil obrigado não desativa); ativar/desativar derruba as outras sessões. **Perdeu o celular**: botão "Redefinir 2FA" (pede a senha de quem redefine): o administrador da prefeitura redefine o de gestor/fiscal da prefeitura dele, o de administrador só o super; o super administrador só pelo servidor (`php artisan admin:redefinir-2fa`). Endpoints: `POST /api/login/2fa`, `POST /api/2fa/iniciar|confirmar|codigos-recuperacao`, `DELETE /api/2fa`, `POST /api/users/{id}/redefinir-2fa`. Front: login em duas etapas, `ativar-2fa` (tela obrigatória), `seguranca` (menu "Segurança da conta"), selos "2FA" e "2FA pendente" na lista de usuários. Nova dependência: `qrcode` (npm, só no chunk da ativação). Testes: PHP 32 novos, Vitest 34 novos, ponta a ponta 4 novos (o 2FA obrigatório é coberto por PHP e Vitest; o e2e roda com a regra desligada).
- **Administrador da prefeitura + senha temporária gerada + auditoria por prefeitura (2026-09-25)**: novo papel `administrador_prefeitura` (o "super administrador" é o antigo `administrador_interno`, só o rótulo mudou). Ele **edita convênios como um gestor** (recebe alertas, tem o sino) e **gerencia as contas da própria prefeitura**: cria, edita, desativa e redefine senha; consulta e exporta (CSV) a auditoria dela. **Não** vê outra prefeitura, não mexe em super administradores, não altera dados da prefeitura, não vê pedidos de contato e **não exclui** (exclusão continua só do super administrador). Não pode se desativar nem mudar o próprio papel (garante que a prefeitura sempre tem um administrador ativo). **Ninguém digita a senha de outra pessoa**: ao criar a conta ou clicar em "Redefinir senha" o sistema gera uma temporária **aleatória** (`XXXX-XXXX-XXXX`), mostrada **uma única vez** num aviso (copiar, "repasse por canal seguro", só fecha pelo botão), guardada só como hash; a troca no primeiro acesso é obrigatória. A senha fixa "nome+1234" foi descartada de propósito: nome e e-mail de servidor público são públicos, então qualquer um entraria antes do dono e escolheria a senha. `POST /api/users/{id}/redefinir-senha`; respostas com a senha têm `Cache-Control: no-store`. A tabela `audits` ganhou `tenant_id` (preenchido sozinho pelos Models e retroativamente por migration, testada no Postgres) e `GET /api/audits/exportar` (mesmo filtro da tela; CSV com nome da prefeitura e rótulos legíveis, neutraliza fórmulas). Menu/rotas: Usuários e Auditoria para os dois administradores; Prefeituras e Pedidos de contato só para o super. Testes: 208 PHP, 119 Vitest, 25 ponta a ponta. **Ainda falta**: 2FA (agora vale para os dois administradores), convite por e-mail no lugar de repassar a senha temporária, validade da senha temporária (hoje ela não expira até o primeiro acesso), Sentry, ambiente de homologação. O e2e cria contas `e2e.*` e as **desativa** ao final (não há exclusão de usuário); no banco de desenvolvimento elas se acumulam: para limpar, `User::where('email','like','e2e.%')->delete()` no tinker.
...
- [ ] **Backups para fora do servidor — código pronto, falta criar o bucket** (2026-09-25): `scripts/backup-nuvem.sh` envia os backups **criptografados no servidor** (AES-256, `BACKUP_SENHA`) para o Cloudflare R2 (ou qualquer S3) via rclone em container; `backup.sh` chama sozinho, com retenção de 30 dias na nuvem (`BACKUP_NUVEM_RETENCAO_DIAS`), e termina com erro se o envio falhar. Ensaiado de ponta a ponta com um S3 de teste: envio, conferência, bucket sem texto aberto, senha errada, nuvem fora do ar e recuperação total (banco + documentos) só pela nuvem. **Validado também contra o R2 real** (2026-09-25; bucket `ctsconveniosbackup`, endpoint `https://<ACCOUNT_ID>.r2.cloudflarestorage.com` sem o nome do bucket): envio criptografado, conferência, listagem, download idêntico e limpeza por idade. O teste real achou um erro que o ensaio local escondia (a limpeza falhava em silêncio por conflito de flags do rclone); corrigido, e agora qualquer falha da limpeza aparece e faz o backup terminar com erro. **Falta:** no dia do deploy, colocar `BACKUP_S3_*` e `BACKUP_SENHA` no `.env.production` do servidor (as chaves estão só no gerenciador de senhas do dono, nunca no repositório) e guardar a senha de criptografia fora do servidor. Opcional: regra de *Bucket Lock* no R2 (prazo menor que 30 dias) contra apagamento por quem invadir o servidor; limitar o token ao IP do servidor.


- [ ] **Revisar os 8 PRs abertos do Dependabot** (2026-09-26; o `dependabot.yml` gerou todos de uma vez no primeiro dia). Fazer **um por vez, esperando a CI ficar verde**, nesta ordem:
  1. **Actions do GitHub** (baixo risco): #8 `actions/checkout` 4→7, #7 `actions/cache` 4→6, #5 `actions/setup-node` 4→7, #1 `actions/upload-artifact` 4→7, #6 `docker/build-push-action` 6→7. Conferir no changelog de cada uma se mudou alguma entrada (`with:`) usada em `.github/workflows/`. Elas também tiram o aviso de "Node.js 20 deprecated".
  2. **`nginx` 1.27→1.29** (#4, `docker/prod`): revisar o `nginx.conf` do servidor de arquivos estáticos e subir a imagem de produção localmente antes de aceitar.
  3. **`node` 22→25** (#2, `docker/prod`): o 25 não é LTS (o CI e o build usam Node 22). Preferir **fechar o PR** e configurar o Dependabot para ignorar versões ímpares/major do Node (ou subir só para a próxima LTS, 24).
  4. **`postgres` 15→18** (#3): **não é só trocar a imagem**: muda o formato do diretório de dados (volume novo), exige `pg_dump` + restauração (ou `pg_upgrade`) e testar o `scripts/backup.sh`/restauração. Fazer só com tempo e backup; até lá, **fechar o PR** e ignorar major do `postgres` no `dependabot.yml`.
  Depois, ajustar `.github/dependabot.yml` (agrupar as actions num único PR e limitar o `open-pull-requests-limit`) para não voltar a receber uma enxurrada de e-mails.
- [x] **CI dos testes de PHP corrigida (2026-09-26)**: o passo chamava `php artisan test --no-interaction`, que o PHPUnit do CI não aceita, então **os testes de PHP nunca rodaram no CI** (falhava em segundos desde o primeiro commit). Agora roda `php artisan test` e, se falhar, o resumo vai numa *annotation* (o log completo do GitHub exige login; a annotation aparece até no e-mail e pela API pública). Os 4 jobs passam.
