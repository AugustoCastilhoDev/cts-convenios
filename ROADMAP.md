...
1. **Upload/download de arquivos (`ArquivoConvenio`)** — Módulo Repositório Digital do doc original. Precisa: configurar disk no `config/filesystems.php`, Form Request com validação de MIME/tamanho, Service com `Storage::putFile`, Controller (index/store/download/destroy), testes.
2. **Gestão de usuários** — CRUD de `users` restrito a Administrador Interno (criar Gestor/Fiscal de uma prefeitura). Inclui o comando `admin:criar` da pendência acima.
3. **Motor de Alertas (Módulo 3 — o diferencial do produto)**:
...
4. **Front-end Vue.js 3 + TailwindCSS (Módulo 2)**:
...
5. **Auditoria/relatórios para o Fiscal de Controle Interno**: endpoint de exportação (a ability `export` já existe na `ConvenioPolicy`, falta o Controller/formato de exportação — CSV/PDF).
6. **Preparação para produção**: revisar `APP_DEBUG`, gerar `APP_KEY` novo, secrets fora do `.env` versionado, CI rodando a suíte de testes a cada push.
