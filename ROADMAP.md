...
- [ ] **Backups para fora do servidor — código pronto, falta criar o bucket** (2026-09-25): `scripts/backup-nuvem.sh` envia os backups **criptografados no servidor** (AES-256, `BACKUP_SENHA`) para o Cloudflare R2 (ou qualquer S3) via rclone em container; `backup.sh` chama sozinho, com retenção de 30 dias na nuvem (`BACKUP_NUVEM_RETENCAO_DIAS`), e termina com erro se o envio falhar. Ensaiado de ponta a ponta com um S3 de teste: envio, conferência, bucket sem texto aberto, senha errada, nuvem fora do ar e recuperação total (banco + documentos) só pela nuvem. **Falta você:** criar o bucket no R2, o token (Object Read & Write só nesse bucket) e preencher `BACKUP_S3_*` e `BACKUP_SENHA` no `.env.production` (passo a passo em `docs/PRODUCAO.md`); guardar a senha fora do servidor.
